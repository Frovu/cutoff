...
module.exports = function(id, energy, callback) {
	fs.readdir(path.join(global.config.instancesDir, id), (err, files) => {
		if (err)
			callback(false);
		const fnpart = parseFloat(energy).toFixed(3).replace('.', '');
		const filename = `Trace${'00000'.slice(fnpart.length)}${fnpart}.dat`;
		const proceed = () => {
			fs.readFile(path.join(global.config.instancesDir, id, filename), (err, data) => {
				if (err) {
					callback(false);
				} else {
					...
					global.log(`Trace. pts=${trace.length} optimized=${threshold?optimized.length:trace.length}`);
		...
		if(!files.includes(filename)) {
			...
			spawnCutoff(id, energy, (code, signal) => {
				global.log(`cutoff(trace) code=${code} sg=${signal} took ${(Date.now()-start)/1000} seconds`);
				if(code === 0) {
					proceed();
					fs.removeSync(path.join(global.config.instancesDir, id, 'Cutoff.dat'));
				} else {
					callback(false);
				}
			});
