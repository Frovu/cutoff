...
router.post('/', async (req, res, next) => {
	...
	try {
		const result = await global.query('select email from users where email=?', [r.email]);
		if(result.length > 0)
			return res.status(409).json({message: 'email already occupied'});
		const hash = bcrypt.hashSync(r.password, 9);
		await global.query('insert into users (email, password) values (?, ?)', [r.email, hash]);
		res.status(201).json({message: 'user created'});
		global.log('User registered: '+r.email);
	} catch(e) {
		next(e);
	}
});
...
router.post('/login', async (req, res, next) => {
	...
	try {
		const result = await global.query('select id, password from users where email=?', [r.email]);
		if(result.length == 0)
		...
		global.log('User logged in: '+r.email);
	} catch(e) {
		next(e);
	}
});
...
router.get('/', async (req, res, next) => {
	try {
		...
		const result = await global.query('select email from users where id=?', [req.session.userId]);
		if(result.length == 0)
...
router.post('/logout', (req, res) => {
	global.log('User logged out, id='+req.session.userId);
